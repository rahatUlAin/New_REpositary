...
<div class="media" >
    ...
    <div class="media-body">
      <h2 class="media-heading" style="color:#01477A" ><b>ACB Co-ordinator</b></h2>
...
$sql1 = "SELECT * FROM reservation join user on reservation.creator_email=user.email where app_by_coor=0";
$result1 = $conn->query($sql1);
function approved($record){
    if($record['app_by_adv'] == 0 || $record['app_by_coor'] == 0){
        return "Not Approved";
    }
    else{
        return "Approved";
...
if ($result1->num_rows > 0) {
      ...
      if(isset($_POST["radio".$count.""])){
        $selected_radio =$_POST["radio".$count.""];
         ...
         if ($selected_radio == 'accepted') {
                $sql2 = "UPDATE reservation SET app_by_coor=1 where id=$row1[id]";
        $conn->query($sql2);
        $approved1="Approved";
 ...
 <h3  style='color: #01477A' align='left'>Respond to Request</h3>
  <form action='coor_manage.php' method='post'>
